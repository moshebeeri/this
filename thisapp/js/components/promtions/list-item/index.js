...
export default class PromotionListView extends Component {
    ...
    cancelPromotion() {
        const {item, businessId, actions} = this.props;
        actions.deletePromotion(item._id, businessId);
    }
                ...
                <View style={[styles.promotion_card, {padding: 5, alignItems: 'center', width: StyleUtils.getWidth()}]}>
                    ...
                    {promotionItem.location ?
                        <View style={[styles.promotionDetailsContainer, {width: StyleUtils.getWidth()}]}>
                            ...
                            <View style={styles.expireDateContainer}>
                                <SubmitButton title={strings.Cancel.toUpperCase()} color={'#e65100'}
                                              onPress={this.cancelPromotion.bind(this)}/>
                            </View>
